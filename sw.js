 ...
 *
 * 缓存策略（v70，手势守卫：阅读页的点词/点句不再只看 click —— 实测手指抖动 ≤12px 时浏览器
 *   照样发 click（12px 发、16px 不发），长按 700ms 松手更是完全不拦，于是「想滚动 / 想长按
 *   选词复制」都会变成「弹出词卡、弹出译文」。现在明显移动或长按的指针序列一律否决，
 *   键盘与读屏路径照常放行。上一条 v69 是句末朗读喇叭退出正文行盒）：
 ...
 */
const CACHE = "wordlens-cache-v70";
