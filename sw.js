 ...
 *
 * 缓存策略（v93，词汇三态定死 + 点句热区扩大 + 阅读设置按钮强化，见 HANDOFF §0.1）：
 *   SW 本体逻辑一字未动 —— 改的是 app.js / styles.css：
 *   ① 词汇三态红线：加入生词本不再撤「已认识」；词库操作（追加/覆盖/删词/清空）
 *      只动 customVocab；已认识 Tab = S.known 全量（未收藏过的给状态卡）；
 *   ② 正文左右留白映射为对应句热区（blankToSentence），句间/段间间隙保持死区；
 *   ③ Aa 阅读设置按钮实色高对比（三主题）+ tune 小图标；active 字色 var(--bg)
 *      修夜间浅紫底白字 2.2:1 的低对比。
 *
 *   上一条 v92 修朗读层错误分类与无语音拦截（阶段 0 设备基线实测驱动）：
 ...
 */
const CACHE = "wordlens-cache-v93";
