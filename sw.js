 ...
 *
 * 缓存策略（v72，朗读层重写：原来只设 u.lang 就 speak()，而 lang 只是**提示**，
 *   浏览器据此自行从系统语音表里挑 —— 本机 Speech_OneCore 只有 3 个中文包、0 个英文，
 *   于是拿默认（中文）voice 念英文，且 onerror 不触发，用户只听到难听、拿不到任何线索。
 *   现在主动挑 voice（按 lang + 名字线索打分）、语速三档、全文朗读改分句串行队列
 *   （原先把 209 句 join(" ") 拼成一条超长 utterance，长文必被引擎掐断），
 *   fab-bar 朗读态加进度 / 暂停 / 停止。上一条 v71 是 AI 栏目封面由渐变改为实图）：
 ...
 */
const CACHE = "wordlens-cache-v72";
