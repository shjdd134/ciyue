...
/* 栏目表。2026-09-24：AI 栏目（Offbook Press）整栏下架 → 从表里撤掉；
 * 「寓言」保留为空入口（历史遗留），与「明星」不同 —— 明星连入口都不给。
 * app.js 两处渲染都会再按「该栏目有没有文章」过滤一遍，所以空栏目不会露出。 */
const CATEGORIES = ["全部","人物","足球","成长","社会","科技","历史","寓言"];
