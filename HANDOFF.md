...
- **当前工作树状态（2026-09-13）**：SW 策略 v43，词库全库 **4,082 词**（基础层 2,069 + 核心层 2,013），文章 **19 篇**（足球 2、AI 3、成长 11、明星 3；寓言入口保留但当前清空）。误执行 `--replace` 后已从远端事故前版本恢复原有 11 篇成长文章及配图，同时保留本次更新的足球、AI、明星文章；19 篇均有本地封面，另有点词翻译层约 38,267 词（阅读页任意单词点击查义）。缓存优先/1 小时新鲜窗/304 协商回落 + 保留上一代缓存作回退；v43 为移除旧背词状态后的发布缓存版本
