> 2026-09-17 当前策略：**RSS 采集已全部停用**（足球于本日停采，全部足球文与封面一并撤下），每日自动采集只剩人物审核队列；成长存量保留但 RSS 暂停，明星采集停用。**2026-09-24 追加：AI 栏目（Offbook Press）整栏下架，人物栏目撤下「梅根·福克斯」** —— 全库 19 → 13 篇。人物明确排除赞达亚、梅根·福克斯。见 [PEOPLE-COLUMN.md](PEOPLE-COLUMN.md)。2026-09-26 晚补充：开发验证完成后用户已授权部署，v90（含私人书架两轮、查词卡合并、Word/PDF 词库导入）已推送 main 上线。
...
**词阅 WordLens**：在线英语精读 App（备考 CET-4），纯静态 HTML/CSS/JS + Service Worker，手机壳布局；Word/PDF 文档导入按需加载随包提供的解析与识别组件。
- **线上**：https://shjdd134.github.io/ciyue/ （GitHub Pages，`shjdd134/ciyue` 仓库 main 分支）

### 私人书架第一轮（2026-09-26 开发，已随 v90 部署；验证状态见下方第二轮）

本轮目标是个人英文书架：先接着读，再读自己挑的文章。代码基于 main `44132ecf9b0d0ace074cf11bc3b9979bbe499434`；GitHub 插件创建分支返回 403，本轮以代码包交付，后经另一会话于 2026-09-26 应用并部署。

- 首页次序：继续阅读 → 书架入口 → 想读（置顶优先）→ 最近发布 → 喜欢 → 摄影精选与偶遇推荐。文章没有入库时间字段，因此本轮明确标为「最近发布」，不把原刊日期冒充入库时间。
- 新增 `wantRead`、`pinnedReads`；打开文章会移出待读队列，继续沿用阅读记录。喜欢复用 `articleFeedback[id].rate === "up"`，可与任何阅读状态共存。已有喜欢会直接显示，新字段进入原有备份、原生镜像和导入校验。
- 单篇评价只影响该篇，不再扩散至同栏目、同来源；书架的下一篇沿用读者列表顺序。
- 修复按篇滚动位置兜底和历史元信息被进度保存覆盖；正文重绘保留选中句与展开译文；点句位移补偿携带退化段的 `rs`。
- 验证：`node tools/nav-test.js` 68/68，深链 2/2，SW 9/9，版本校验与 JS 语法检查通过。完整 audit 因本地缺少字体、图片等资源未跑完；没有可用的浏览器运行时，未完成手机视口及真机验收。
- 合入前在完整仓库运行 `node tools/audit.js`；在手机上检查收藏不误开文、列表返回位置、查词后继续读、切中文/字号、长文续读及备份导入导出。文章正文和封面数据未修改。

### 私人书架第二轮（2026-09-26 开发，已随 v90 部署）

本轮在第一轮基础上加入用户选定的四项功能。写作时用户明确要求「先不部署」，未推送 main；后续已随 v90 于 2026-09-26 部署上线。

- 阅读意图：`readingIntent` 保存「随心阅读 / 辅助学习」；随心模式隐藏需学词数、考试倒计时和正文学习高亮，保留查词、翻译、收藏。首次使用默认为随心，已有用户与旧备份沿用辅助学习；切换不改词汇来源、认识词、生词本或中文偏好。
- 封面与书架：`leadPhotoKey` 可固定首页现有摄影池的一张、恢复随机或换图，署名与来源链接不变；`collections` 保存自定义名称和文章引用，支持创建、改名、多书架归属、移出及删除确认。删除书架保留喜欢、想读和历史；文章下架后仍保留其书架引用。
- 续读与重读：有阅读记录的文章显示「继续上次位置 / 从头重读」。重读只重置这一遍的位置；`readHistory[id].inProgress` 让已读文章也能出现在「在读」中，同时保留历史、喜欢和词汇。再次读完可累计完成次数，重复点击不重复计数。统一 `openReadingArticle` 先结算上一页，书架来源顺序进入导航快照。
- 句子与感想：选中正文句子后点书签，可保存双语快照和可选感想；「我的 → 句子与感想」可编辑、确认删除和返回原句。`sentenceNotes` 保留文章标题、原句、译文与坐标；文章下架后仍可看快照，段落移动后按文本重定位，原句不再存在时明确禁用原文入口。
- 新字段全部进入现有 JSON 备份和原生存储镜像，补充校验并兼容旧备份；未修改文章、译文或封面数据。
- 集成复核补充：书架连读到已读文章时进入重读中；删除书架正确消费详情导航层；书架和封面写入失败会回滚并保留输入，句子收藏回滚时同步镜像；词表异步到达不关闭正在编辑的感想或书架名称。
- 新增回归脚本：`reading-intent-test.js`、`collections-test.js`、`reread-test.js`、`sentence-notes-test.js`。最终测试结果见交付包 `validation.log`；完整资源已经按 main 原始 blob 补齐，第一轮缺资源的审计障碍已解除。
- 验证限制：浏览器测试环境拒绝打开本地开发地址（`ERR_BLOCKED_BY_CLIENT`），未完成真实浏览器、手机视口与真机验收。合入上线前需在手机实测选句收藏、中文展开后定位、切模式/字号、书架进出、长文续读以及备份导入导出。

### 查词卡合并（2026-09-26，本地 v89，已随 v90 部署）

- 按用户截图把 WORDS 词卡两层合成一层：首次点击即显示词形、完整释义、本文原句及词库例句，移除「更多」与 `sheetMore` 状态。
- 底部固定「加入生词本 / 移出生词本」和「已认识 / 取消已认识」；正文与例句在卡片内部滚动。标认识仍使用就地 `paintWord`，保留阅读位置、生词记录和翻译展开状态；关闭或完成操作同时清除查词定位底色。
- 例句仍按需加载，在首次打开缺例句的词卡时请求；失败可重试，关卡或换词后异步结果只回填对应槽位，不重绘正文。词库外卡没有例句槽，不请求例句文件。
- 词卡显示完整的被点句子与译文（含拆分句 `si + rs` 定位）；生词本保存时仍裁为目标词附近短语境，不改变原有短摘录策略。
- 新增 `tools/lookup-card-test.js`，同时把原审计中的两级词卡预期更新为首次点击直接显示。延续此前不部署要求；浏览器和真机视觉验收尚未完成。

### Word / PDF 词库导入（2026-09-26，本地 v90，已部署）

- 「我的词库 → 导入单词」支持 Word `.docx`、文字版 PDF 和扫描版 PDF 英文 OCR；保留 TXT / CSV / JSON 入口。旧 `.doc` 给出另存为 `.docx` / PDF 指引；Word 内嵌图片暂不 OCR，会提醒。
- 文件在设备上解析，不上传文档；扫描页 OCR 可关闭，有进度和取消。识别只填写可编辑草稿，沿用去重统计与追加/二次确认覆盖；例句和英文说明也可能成为候选，界面提示核对。不是词义导入或文档翻译功能。
- 单文件最多 20 MB；PDF 最多 100 页，其中 OCR 每次最多 20 页；文本最多 200 万字符 / 10 万候选词。密码 PDF 明确要求先解除密码；损坏、无英文、组件加载失败不改原词库或原草稿。
- 取消、离页、重选文件用 AbortController 和序号拦截迟到结果。SW 更新在识别或尚有草稿期间延期，延时刷新执行前再次检查，避免后台更新丢失核对中的草稿。
- 解析入口为 `assets/vocab-import.mjs`；DOCX 使用本地 JSZip 3.10.1，PDF 使用 PDF.js 5.6.205 legacy，扫描页使用 Tesseract.js 7.0.0 和本地英文模型。组件、worker、字体、CMap、WASM、模型及许可证均在 `assets/vendor/`，按需加载，Android 白名单包含完整依赖。网页首次识别需要联网加载组件；原生壳随包提供。
...
### 词汇三态定死 + 点句热区 + 阅读设置按钮（2026-09-26，v93，已部署）

- **词汇三态红线（用户定死的产品规则）**：已认识 > 生词本 > 当前词库高亮，三者互不删数据。① `add-note` 不再删 `S.known`（旧实现收藏即撤认识）——一词可同时在生词本与已认识里，正文按已认识显示；② 词库操作（追加 / 覆盖 / 删词 / 清空）只动 `customVocab`，换词库、清词库、半年后重导，个人状态原样；③ 词汇页口径重做：生词 Tab = `notebook − known`，已认识 Tab = **`S.known` 全量**（新增 `knownRowHTML` 朴素状态卡；词典查不到的词不整行消失，`nbRowHTML` 也有无释义兜底行）。
- **点句热区**：正文左右 `--rd-pad` 留白点击按 Y 坐标映射给「盖住这个高度的句子」（`blankToSentence`），等同点句；句间 10px / 段间 24px 间隙保持死区（用户明确要求不做模糊区）。`para-peek` 逻辑抽成 `toggleSentencePeek()` 两条入口共用。点词 / 句末喇叭 / 收藏句各带 `data-act`，在 `closest()` 即被接走，不会误触译文。
- **Aa 阅读设置按钮**：实色底盘（`var(--text-1)`）+ 高对比字（`var(--bg)`）+ 淡边框 + 10px `tune` 滑杆小图标（新图标，非齿轮）。★ editorial.css 全站覆盖 token——阅读页里 `--text-1` 实际是墨绿 `#1c2c25`，不是 styles.css `:root` 的 `#12121F`；按钮天然带阅读页纸墨配色。修了 active 态真问题：白字在夜间浅紫 `--brand` 上只有 2.2:1，改 `var(--bg)` 后三主题默认态 11.7–16.4:1、active 态 7.5–8.3:1。
- 守卫：`audit.js` 新增 9 条词汇三态守卫（总数 487/0）；[R3] 三条源码断言随 `toggleSentencePeek` 抽取改锚函数体。`nav-test.js` 空态文案断言更新为「还没有已认识的词」。
- 真浏览器回归：`.bak/probe-vocab-triad.cjs`（不入库）23/23 —— 三态持久化 8 场景（含 reload、覆盖/清空/重导词库）、热区 7 场景（含死区与误触对照）、三主题按钮对比度测量 + 截图。探针踩坑：`getBoundingClientRect` 是视口相对坐标，长文里目标句必须先 `scrollIntoView` 再点。
...
| 项 | 值 |
| --- | --- |
| 文章 | **23 篇 = 成长 10 + 人物 5 + 足球 5 + 社会 1 + 科技 1 + 历史 1**（AI 栏目 5 篇 + 人物「梅根·福克斯」1 篇 = 6 篇于 2026-09-24 下架；2026-09-25 新增拉什福德 + 狄德罗效应 2 篇；**2026-09-26 书架批次新增 8 篇**，见下「书架 8 篇」行） |
| 句子 / 词数 | **4,704 句 / 67,891 词** |
| 封面 | 70 张（本地 `assets/covers/`；远端 blob 总数请跑 `tree-diff`） |
| 发布基线 | `.bak/published.json` = **`8c90dfb`**（2026-09-26 晚，「中英句对修复合并部署 v91」，19 文件；同日更早：`700a9cb` v90 整包 239 文件）· ★ 本行自身的改动会再引出一个 commit，**以 `node tools/doc-numbers.mjs` 打出的实测值为准** |
| 资源版本 | `?v=93` · SW 缓存名 `wordlens-cache-v93`（v93 = 2026-09-26 词汇三态定死：收藏不撤认识 / 词库操作不碰个人状态 / 已认识 Tab=`S.known` 全量 + 点句热区扩大（左右留白映射对应句、间隙死区）+ Aa 阅读设置按钮实色高对比；v92 = 2026-09-26 书架批次：8 篇精翻入库 + 社会/科技/历史三新栏目 + shelf 通道；v91 = 同日中英句对修复：679 条译文校对 + 47 组 `alignedParts`，废弃 cn-dup 显示机制；v90 Word/PDF 词库导入 + 单层查词卡 + 私人书架同日早些时候已部署） |
...
| 采集策略 | **RSS 采集已全部停用**；每日自动采集只剩人物审核队列（≤1 篇）。**AI 栏目已于 2026-09-24 整栏下架**（`tools/offbook.mjs` 的 `ESSAYS` 摘空，链路仍在但不再产出）；旧明星停用。**2026-09-26 新增 `tools/shelf.mjs` 书架通道**（用户点名篇目、全人工精翻、无自动采集，与 james-clear.mjs 同构） |
| 成长 10 篇 | Dan Koe：`gr-how-to-fix-your-entire-life-in-1-day`；James Clear《The Diderot Effect》（`gr-james-clear-diderot-effect`，2026-09-25 入库，`tools/james-clear.mjs` 通道）；Paul Graham 三篇（`gr-pg-what-youll-wish-youd-known` / `gr-pg-how-to-do-what-you-love` / `gr-pg-how-to-do-great-work`，原社区译本署名：lzwjava / 王亮 / untymen.com；2026-09-26 逐句校对，《How to Do What You Love》重译，署名保留原来源并注明词阅修订，修订持久来源见 `tools/sentence-translations-reviewed.json`）；**书架批次 5 篇**（见下行） |
| 书架 8 篇（2026-09-26，`tools/shelf.mjs` 通道） | 用户私人书架点名的 8 篇，全部**词阅精翻**（`translation_type:"ciyue_edited"`，741 句）。成长 5 篇（常青，免 pin）：Aeon `gr-aeon-instrumentalisation`（200 句，2 张文内图带译注）、Psyche `gr-psyche-pretesting`（68 句）、Sivers `gr-sivers-building-without-predicting`（102 句，含 2 条 blockquote 引言 + 3 条步骤清单）、Greater Good `gr-greatergood-rustout`（86 句，4 个 h2 小节 `head:2`）、Psyche `gr-psyche-relationship-ambivalence`（71 句）。新栏目 3 篇（**带 `pin:true`** 豁免 30 天过期闸与配额，qc F2 同样认 pin）：社会 `so-bbc-kindness-trolls`（94 句）、科技 `tech-npr-navier-stokes`（68 句，2 个 h3 小节）、历史 `hist-bbc-markov-umbrella`（61 句）。BBC 系（bbc.com/ichef/ychef）在本网络被 DNS 级拦截，抓取走**本机代理 `127.0.0.1:7892`**（系统代理，`reg query Internet Settings` 实测；curl 不读 IE 代理必须显式 `-x`）；NPR 头图的 brightspotcdn 403，改抓 `url=` 参数里的 S3 源文件后本地压缩。BBC 的 image-block 即头图（与 og:image 同 ID），按 v65 的 sameAsCover 规则只做封面不内嵌 |
| 新栏目（社会 / 科技 / 历史） | 2026-09-26 随书架批次创建。三处联动：① `assets/data.js` 的 `CATEGORIES`（顺序：全部/人物/足球/成长/社会/科技/历史/寓言）② `assets/app.js` 的 `CAT_META`（新图标 `globe`/`chip`/`hourglass`，同批删掉了 AI 的死条目）与 `CAT_BLURB` ③ `tools/doc-numbers.mjs` 的 id 前缀契约（`so-`/`tech-`/`hist-`）。**非常青栏目**：文章必须 `pin:true`（publish 30 天闸 + qc F2 的豁免都认 pin） |
...
| AI 栏目（已下架） | **2026-09-24 整栏撤下**（用户决定）。原为 Offbook Press 官方中英双语长文（Dawei Geng，`ob-*`）一期一整篇 5 期。**撤干净需要三处同时在场**（缺一个就会以某种路径回来）：① `tools/offbook.mjs` 的 `ESSAYS = []`（5 期移入 `RETIRED_ESSAYS` 仅存记录）② `tools/ingest.mjs` 的 `DROP_LIST`（5 个 id）③ `tools/content-scope-test.mjs` 的断言已**反转**（原「AI 应已由 ob-* 上线」→ 现「库里不得再出现 AI / `ob-*`」，同时 `CATEGORIES` 不得含 `AI`）。附带的连带改动：`assets/app.js` 的 `LEAD_PHOTO_POOL` 与 `tools/audit.js` 的 `IMG_SIZE` 同步删掉 megan-fox 5 项；`audit.js` 的 `[R2]`（节标题）2026-09-24 曾因「线上已无样本」改为合成桩 + 显式断言「库内 head 段数 = 0」；**2026-09-26 书架批次带回真实样本（GG 4 个 h2 + NPR 2 个 h3 = 6），断言已翻回钉数 6** |
| 寓言 | 入口保留，当前为空 |
