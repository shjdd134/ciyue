...
>   - ⑧ 图片文件本身（`assets/covers/gr-...-3.jpg` / `-4.jpg`，共 130KB）**本批没删** ——
>     删文件要走 publish 的 `delete[]` 路线，而那条路会同时改写权威基线 `articles[]`，
>     本仓库有并行 agent 改数据时会把中途状态判成「都不要了」（历史实测删过 43 项）。
>     摘掉引用后这两张已无人访问，纯属死重，留作后续单开一轮清理。
