...
eq('筛选为时尚', at().cat, '时尚');
const fashion = ctx('ARTICLES.filter(a=>a.cat==="时尚").map(a=>a.id)');
ok(`时尚分类有 ${fashion.length} 篇`, fashion.length > 1);

click({ article: fashion[0] });
eq('进入阅读页', at().view, 'read');
eq('来路压栈一层', at().depth, 1);
ok('读完卡片有去处按钮', /finish-nav/.test(screenEl.innerHTML) && /返回时尚/.test(screenEl.innerHTML));

click({ act: 'go-back' });
eq('返回回到发现页（而不是首页）', at().view, 'discover');
eq('分类筛选被保留', at().cat, '时尚');
eq('栈已清空', at().depth, 0);
...
click({ tab: 'discover' });
click({ cat: '时尚' });
const ids = ctx('ARTICLES.filter(a=>a.cat==="时尚").map(a=>a.id)');
click({ article: ids[0] });
...
click({ act: 'go-back' });
eq('连读几篇后返回仍回到分类页', [at().view, at().cat], ['discover', '时尚']);
