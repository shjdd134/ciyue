 ...
 *   2026-09-22 深夜两次瞬时读数都表现为「差额 98」，但**来源不同**：
 *   ① 远端侧：响应残缺时拿到 174 个 blob（正常 273）→ 99 个文件被判成「本地有、远端没有」；
 *   ② 本地侧：`.gitignore` 的 `mobile/www/` 那条规则没生效 → 本地读数 292 → **390**
 *      （多出来的正是 `mobile/www` 的 98 个文件），差额 98 与 ① 长得一模一样。
 *   两次都无法复现出稳定的触发条件，所以这里不猜成因，只做两件事：
 *   把「不完整」当场暴露出来（下面前两道守卫），并让 tree-diff 对本地侧也断言一次
 *   （构建产物绝不许出现在对账列表里）。**读数偏了就必须响亮地失败，而不是多出 98 项。**
 *   守卫：
