/* James Clear 通道「正文完整性」对账 —— **独立口径**，不 import 任何项目解析代码。
 *
 * 与 tools/football-coverage.mjs 同一铁律（2026-09-16 的教训）：
 *   拿提取器去验它自己抓的正文，等于「尺子量掉的内容让尺子自己报」—— 它一定报「全对」。
 *   本脚本**完全不认识 jamesclear.com 的结构**，只做一件事：
 *     把原文 HTML 拆成块级文本行 → 每行取 6-gram → 去「已提取的英文」里找，找不到就是缺口。
 *   同一件事反过来再做一遍（提取物 → 原文），找超译 / 污染 / 凭空出现的句子。
 *
 * 与 football-coverage 的差别只有两处：raw 目录、导航特征词（jamesclear.com 的站务块）。
 * 其余逻辑刻意保持逐字同构 —— 两把尺子必须长得一样，横向比较才有意义。
 *
 * 缺口四类，**只有 `句` 值得追**：
 *   nav  导航 / 订阅 / 版权特征 —— 本来就该缺
 *   短    词数 < 6
 *   碎片  ≥6 词但无句末标点 —— 标题 / 条目 / 半句
 *   句    ≥6 词且有句末标点 —— **这才是正文缺口**，`--strict` 时非零退出
 *
 * 用法：
 *   node tools/james-clear-coverage.mjs --all
 *   node tools/james-clear-coverage.mjs --id <id> [--verbose] [--strict]
 */
...
const RAW = path.join(ROOT, 'tools', '_james-clear', 'raw');
...
const ENT = { amp: '&', lt: '<', gt: '>', quot: '"', apos: "'", nbsp: ' ', ldquo: '“', rdquo: '”', lsquo: '‘', rsquo: '’', mdash: '—', ndash: '–', hellip: '…', eacute: 'é' };
...
/* ---------------- 缺口分类 ----------------
 * 导航特征词是本站（jamesclear.com）实测的站务块：订阅话术、社媒、页脚、相关文章。 */
const NAV_RE = /(©|all rights reserved|subscribe|newsletter|sign up|cookie|privacy|terms of use|follow us|share this|advertisement|sponsored|read more|watch now|comments?\b|log ?in|sign ?in|download the app|more from|related|recommended|click here|email address|james clear writes about|habit\.com)/i;
const SENT_END = /[.!?]["')\]]?$/;
...
console.log(`\n合计「句」类缺口 ${sentGaps} 条${sentGaps ? '  ← 必须逐条确认是导航/订阅框/页脚，还是真丢正文' : ''}`);
